...
| 标签栏空 | ChatArea：TabBar 标题显示 "Chat"，内容区居中显示 "Start a session to chat"，隐藏 ChatInput；FileEditor：TabBar 标题显示 "Preview"，内容区显示 "Select a file to preview" |
...
**Per-session 流式事件操作**（新增，用于 `setupWailsEvents` 按 `session_id` 分发）：

| 操作 | 用途 |
|---|---|
| `updateSessionMessage(id, delta)` | 追加流式文本到 `sessionMessages[id]` 的最后一条 assistant 消息；若无 assistant 则创建 |
...
| `updateSessionToolDone(id, name, output, status)` | 更新 tool 状态；若无 assistant 则创建新消息载入 tool |
| `addSessionError(id, content)` | 后端流式错误时写入 `sessionMessages[id]`，非活跃会话也能在切回时显示错误 |
...
- 无活跃会话时：TabBar 标题显示 "Chat"，内容区垂直居中显示 "Start a session to chat"，ChatInput 不渲染
- 有活跃会话但无消息时：显示 "No messages yet. Start a conversation."，ChatInput 正常渲染
...
- 切换标签：
  1. 检测缓存中 EditorView 的文档是否与 store 中内容一致，不一致则 `view.dispatch` 更新
  2. 重新挂载 DOM + 调用 `editorView.requestMeasure()` 强制重新计算布局
  3. 切换容器可见性
- 关闭标签：destroy 对应实例，从 Map 移除，移除 DOM 容器
- **LRU 上限**：最多缓存 10 个 EditorView 实例。超限时 destroy 最久未访问的实例
- **Unmount 清理**：组件销毁时遍历 destroy 所有 EditorView 实例，防止布局切换时泄漏
- **滚动条**：`.cm-editor { height: 100% }` 确保编辑器撑满容器，`.cm-scroller` 显式匹配全局滚动条样式
...
| 文件 | 变更 |
|---|---|
| `frontend/src/store/index.ts` | 新增 11 个状态字段和 19 个操作（含 5 个 per-session stream actions）；移除 4 个旧字段/操作；流式路由按 `session_id` 重构 |
| `frontend/src/components/TabBar/TabBar.tsx` | 新增：标签栏、溢出下拉（ResizeObserver）、状态指示、键盘导航、ARIA、常量提取 |
| `frontend/src/components/Chat/ChatArea.tsx` | 移除 header bar → 集成 TabBar；消息缓存读写；流式路由重构；空状态优化（无会话时隐藏 ChatInput） |
| `frontend/src/components/Chat/ConfirmModal.tsx` | 新增 `confirmLabel` prop；通用错误文案 |
| `frontend/src/components/FileTree/FileEditor.tsx` | 移除 header bar → 集成 TabBar；多 DOM 容器 + CodeMirror LRU 缓存 + 内容同步 + 脏关闭拦截 + unmount 清理 |
| `frontend/src/components/FileTree/FileTree.tsx` | `handleFileClick` 改为 `openFileTab`；`activeFilePath` 替换 `selectedFilePath` |
| `frontend/src/components/Sidebar/SessionList.tsx` | `handleSelect`/`handleNewSession`/`handleDeleteConfirm` 改为调用 `openSessionTab`/`closeSessionTab` |
| `frontend/src/components/StatusBar/StatusBar.tsx` | `generating` boolean 改为 `generatingSessionId !== ''` |
| `frontend/src/components/TitleBar/TitleBar.tsx` | `Application.Quit()` → `Window.Close()` |
| `frontend/src/index.css` | `.cm-editor { height: 100% }` + `.cm-scroller` 显式滚动条样式 |
| `frontend/src/App.tsx` | 无结构性变更 |
