"""extend events table with M3 Memory Web fields

Revision ID: 012_extend_events_with_m3_fields
Revises: 011_extend_checkpoints_for_m3
Create Date: 2026-02-22

This migration adds M3 Memory Web extensions to the events table:
...
These fields enable:
- Event ordering and replay through sequence numbers
- Event categorization for filtering and search
- Full narrative tracking with input/output
- Cross-device timestamp synchronization
- Flexible tagging and checkpoint association
"""
...
revision: str = '012_extend_events_with_m3_fields'
down_revision: Union[str, Sequence[str], None] = '011_extend_checkpoints_for_m3'
...
def upgrade() -> None:
    ...
    if db_type == 'postgresql':
        ...
        op.execute("DROP INDEX IF EXISTS idx_events_description_gin")
        op.execute("""
            CREATE INDEX idx_events_fulltext_gin ON events USING gin(
                to_tsvector('english',
...
def downgrade() -> None:
    ...
    if db_type == 'postgresql':
        ...
        # Restore original full-text index
        op.execute("""
